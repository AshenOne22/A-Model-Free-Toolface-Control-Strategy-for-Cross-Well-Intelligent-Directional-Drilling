...
df = read_csv('b_data_ori.csv',
              usecols=['ACTIME', 'TIME', 'TARSTF', 'SGTF', 'dif', 'TOR_P', 'TOR_N', 'DATE', 'ss2'])
data = df.values
length = len(data)
date = data[..., 8]
...
for i in range(length):
    ...
    if i == 0:
        ...
        if abs(temp - date[i - 1]) < 3000:
            temp_time = np.array([data[i, 8] - data[i - 1, 8]])
            temp_data = np.concatenate((data[i], temp_time))
            group[group_num].append(temp_data)
            group_index[group_num].append(i)
        # 时间差大于600秒，数据不连续
...
cols = ['ACTIME', 'TIME', 'TARSTF', 'SGTF','dif', 'TOR_P', 'TOR_N', 'DATE', 'ss2', 'time_interval']
