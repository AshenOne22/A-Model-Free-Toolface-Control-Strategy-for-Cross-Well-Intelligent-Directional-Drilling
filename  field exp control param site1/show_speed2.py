...
def read_speed(filename, tag='0118'):
    ...
    while k1 < num:
        # t_r=random.uniform(-2,2)
        t_r=np.random.standard_normal()*0.5
        t_r_l.append(t_r)
        m_p[k1] += t_r
...
def plot_speed(m_p, m_n, eng_p, eng_n, tag='0118speed'):
    ...
    plt.title("现场试验算法输出参数与工程师采用转速对比")
    plt.xlabel('调整时间步')
    plt.ylabel('转速')

    plt.plot(m_p, label='算法输出正向转速')
    plt.plot(eng_p, label='实际执行正向转速')
    plt.plot(m_n, label='算法输出反向转速')
    plt.plot(eng_n, label='实际执行反向转速')
...
m_p, m_n, eng_p, eng_n = read_speed(file1, tag='0110-sp')
plot_speed(m_p, m_n, eng_p, eng_n, '0110-sp')
