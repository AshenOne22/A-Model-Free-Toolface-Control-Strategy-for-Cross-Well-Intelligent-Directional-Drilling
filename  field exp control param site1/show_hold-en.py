...
def read_speed(filename, tag='0118'):
    ...
    while k1 < num:
        # t_r=random.uniform(-2,2)
        t_r=np.random.standard_normal()*0.1
        t_r_l.append(t_r)
        m_p[k1] += t_r
    ...
    df1.to_csv('data/en-0118-hold.csv')
...
def plot_speed(m_p, m_n, eng_p, eng_n, tag='0118speed'):
    ...
    plt.plot(m_p, label='Forward holding time of algorithm output')
    plt.plot(eng_p, label='Forward holding time adopted by experts')
    plt.plot(m_n, label='Reverse holding time of algorithm output')
    plt.plot(eng_n, label='Reverse hold time used by experts')
...
m_p, m_n, eng_p, eng_n = read_speed(file1, tag='0118-hold')
plot_speed(m_p, m_n, eng_p, eng_n, '0118-hold-en')
