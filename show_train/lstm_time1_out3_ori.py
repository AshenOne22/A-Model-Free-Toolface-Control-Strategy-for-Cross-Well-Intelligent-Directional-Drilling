...
def create_model(look_back, out_dim):
    ...
    model.add(layers.LSTM(8, input_shape=(look_back, in_dim), return_sequences=False))
...
def train_model(model_filepath, epochs_num=100, batch_size_num=64, load=False):
    ...
    history = model.fit(x_train, y_train, batch_size=32, epochs=epochs_num, validation_split=0.1, callbacks=callbacks_list)
