...
def create_model(look_back, out_dim):
    ...
    att_l = layers.Attention()([l2, l2])
    merge = layers.Concatenate()([att_l, l2])
    mlp = layers.Dense(64)(merge)
    out = layers.Dense(out_dim)(mlp)
...
def draw_predict(scalerx, scalery, x, y):
    ...
    for i in range(dim):
        ...
        plt.plot(y_pre[:100,0, i], label=label_ypre)
    plt.legend()
    plt.title('testy')
    ...
    for i in range(dim):
        ...
        plt.plot(ori_y_pre[:100,0, i], label=label_ori_ypre)
    plt.legend()
    plt.title('ori_testy')
