...
def create_model(look_back, out_dim):
    ...
    model.add(layers.LSTM(64, input_shape=(look_back, in_dim), return_sequences=True))
...
def draw_predict_tf1(x, y):
    ...
    # plt.xticks(fontsize=20)
    # plt.yticks(fontsize=20)  # 设置坐标标签字体大小
    plt.savefig(os.path.join(imagePath, 'font2' + param_name))
    plt.show()
    ...
    # plt.xticks(fontsize=20)
    # plt.yticks(fontsize=20)  # 设置坐标标签字体大小
    plt.savefig(os.path.join(imagePath, 'font3' + param_name))
    plt.show()
...
if __name__ == '__main__':
    ...
    if iftrain:
        # 训练模型
        history = train_model(model_path, epochs_num=400, load=False)
        ...
        x_c, y_c = transform_test(scalerx, scalery, data_x, data_y)
        draw_predict_tf1(x_c,y_c)
    else:
        filename = 'lstm1.h5'
