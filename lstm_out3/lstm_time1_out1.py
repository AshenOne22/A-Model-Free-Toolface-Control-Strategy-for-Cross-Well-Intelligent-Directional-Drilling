...
def draw_predict(scalerx, scalery, x, y):
    ...
    # 画图
    plt.figure()
    plt.plot(y[:100], label='label_y')
    plt.plot(y_pre[:100], label='label_ypre')
    # for i in range(dim):
    #     label_y='y'+str(i)
    ...
    # 画图
    plt.figure()
    plt.plot(ori_y[:100], label='label_ori_y')
    plt.plot(ori_y_pre[:100], label='label_ori_ypre')
    # for i in range(dim):
...
def draw_predict_ch(scalerx, scalery, x, y):
    ...
    plt.plot(ori_y[:100], label='真实值')
    plt.plot(ori_y_pre[:100], label='预测值')
    plt.legend()
    plt.title('ori_testy')
    plt.ylabel('工具面状态（°）')
    # plt.xlabel('测试样本点')
...
if __name__ == '__main__':
    ...
    # 画图
    draw_predict_ch(scalerx, scalery, x_test, y_test)
