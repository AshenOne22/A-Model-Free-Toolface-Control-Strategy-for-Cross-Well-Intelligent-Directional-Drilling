...
from demo.attention import Attention
...
def create_model(look_back, out_dim):
    l1 = layers.Input(shape=(look_back, in_dim))
    l2 = layers.LSTM(128, return_sequences=True)(l1)
    l3 = layers.LSTM(128)(l2)

    att_l = Attention(query_dim=64, key_dim=64, value_dim=128, output_dim=128, num_of_head=2)(l3)
    # mlp = layers.Dense(64)(att_l)
    out = layers.Dense(out_dim)(att_l)
...
def train_model(model_filepath, epochs_num=100, batch_size_num=64, load=False):
    ...
    # 保存分数
    s_scores = str(scores)
    s_name = str(model.metrics_names)
    print(s_scores)
    with open(os.path.join(imagePath, "test.txt"), "a") as f:
        f.write(s_name)
        f.write(s_scores)  # 自带文件关闭功能，不需要再写f.close()
...
def draw_predict2(x, y):
    ...
    # 画图
    param_list = ['正向扭矩', '反向扭矩', '正向转速', '反向转速', '正向保持时间', '反向保持时间']

    plt.figure()
    label_y = '真实值'
    label_ypre = '预测值'
    plt.plot(y[:100, 0], label=label_y)
    plt.plot(y_pre[:100, 0], label=label_ypre)
    plt.legend()
    param_name = param_list[0]
    plt.ylabel(param_name)
    plt.title(param_name)
    plt.savefig(os.path.join(imagePath, param_name))
    plt.show()

    plt.figure()
    label_y = '真实值'
    label_ypre = '预测值'
    plt.plot(y[101:200, 0], label=label_y)
    plt.plot(y_pre[101:200, 0], label=label_ypre)
    plt.legend()
    ...
    label_y = '真实值'
    label_ypre = '预测值'
    plt.plot(y[:100, 1], label=label_y)
    plt.plot(y_pre[:100, 1], label=label_ypre)
    plt.legend()
    param_name = param_list[2]
    plt.ylabel(param_name)
    plt.title(param_name)
    plt.savefig(os.path.join(imagePath, param_name))
    plt.show()
    ...
    label_y = '真实值'
    label_ypre = '预测值'
    plt.plot(y[101:200, 1], label=label_y)
    plt.plot(y_pre[101:200, 1], label=label_ypre)
    plt.legend()
    param_name = param_list[3]
    plt.ylabel(param_name)
    plt.title(param_name)
    plt.savefig(os.path.join(imagePath, param_name))
    plt.show()
    ...
    label_y = '真实值'
    label_ypre = '预测值'
    plt.plot(y[:100, 2], label=label_y)
    plt.plot(y_pre[:100, 2], label=label_ypre)
    plt.legend()
    param_name = param_list[4]
    plt.ylabel(param_name)
    plt.title(param_name)
    plt.savefig(os.path.join(imagePath, param_name))
    plt.show()
    ...
    label_y = '真实值'
    label_ypre = '预测值'
    plt.plot(y[101:200, 2], label=label_y)
    plt.plot(y_pre[101:200, 2], label=label_ypre)
    plt.legend()
    param_name = param_list[5]
    plt.ylabel(param_name)
    plt.title(param_name)
    plt.savefig(os.path.join(imagePath, param_name))
    plt.show()
...
def draw_predict_font(x, y):
    ...
    plt.savefig(os.path.join(imagePath, 'font' + param_name))
    plt.show()
...
def draw_compare(x, y, tag='t1'):
    ...
    plt.plot(y[:100, 0], label='Reference state')
    plt.plot(y_pre1[:100, 0], label='改进LSTM')
    plt.plot(y_pre2[:100, 0], label='LSTM')
    plt.plot(y_pre3[:100, 0], label='RNN')
    plt.legend()
    plt.ylabel('工具面状态（°）')
    plt.xlabel('测试样本点')
    plt.title('模型对比结果')
    plt.savefig(os.path.join(imagePath, tag + 'testy.jpg'))
    plt.show()

    plt.figure(dpi=600)

    plt.plot(y[:50, 0], label='Reference state', color='red')
    plt.plot(y_pre1[:50, 0], label='Improved LSTM', color='limegreen')
    plt.plot(y_pre2[:50, 0], label='LSTM', color='darkorange')
    plt.plot(y_pre3[:50, 0], label='RNN', color='b')
    plt.legend()
    plt.ylabel('tool face state (°)')
    plt.xlabel('time steps')
    plt.title('Model Comparison results')
    plt.savefig(os.path.join(imagePath, tag + 'testy2.jpg'))
    plt.show()


def draw_compare_en(x, y):
    y_pre1 = model1.predict(x)
    y_pre2 = model2.predict(x)
    y_pre3 = model3.predict(x)
    y = y * 1000
    ...
    plt.plot(y[:100, 0], label='reference state', linewidth=3.0)
    plt.plot(y_pre1[:100, 0], label='multi-head self-attention LSTM', linewidth=3.0)
    plt.plot(y_pre2[:100, 0], label='LSTM', linewidth=3.0)
    plt.plot(y_pre3[:100, 0], label='RNN', linewidth=3.0)
    ...
    plt.legend(fontsize=20)
    # plt.title('模型对比结果')
    plt.savefig(os.path.join(imagePath, 'testy-en.jpg'))
    plt.show()

    plt.figure()

    plt.plot(y[:50, 0], label='reference state', linewidth=3.0)
    plt.plot(y_pre1[:50, 0], label='multi-head self-attention LSTM', linewidth=3.0)
    plt.plot(y_pre2[:50, 0], label='LSTM', linewidth=3.0)
    plt.plot(y_pre3[:50, 0], label='RNN', linewidth=3.0)
    ...
    plt.grid()
    plt.legend(fontsize=20)
    plt.savefig(os.path.join(imagePath, 'testy2-en.jpg'))
    plt.show()
...
if __name__ == '__main__':
    ...
    action_range = [-2, 2]
    t_name = 'compare'
    path = '_'.join(['timestep', str(look_back), 'outdim', str(out_dim), 't', str(t_name)])
    ...
    # 构建模型
    model1 = create_model(look_back, out_dim)
    model2 = create_model(look_back, out_dim)
    model3 = create_model(look_back, out_dim)
    # # 对测试数据的Y进行预测
    # testPredict = model.predict(x_test)
    # 训练模型
    # history = train_model(model_path, epochs_num=150, load=False)
    model1_path = 'model/timestep_1_outdim_3_t_7/lstm1.h5'
    model1.load_weights(model1_path)
    model2_path = 'model/timestep_1_outdim_3_t_6/lstm1.h5'
    model2.load_weights(model2_path)
    model3_path = 'model/timestep_1_outdim_3_t_8/lstm1.h5'
    model3.load_weights(model3_path)

    # 缩放数据
    scalerx, scalery, x_c, y_c = transform_train(data_x, data_y)
    draw_compare(x_c, y_c, tag='t2')
    draw_compare(x_test, y_test)

    # 画图
    # draw_history(history)
    # draw_predict(scalerx, scalery, x_test, y_test)
    # draw_predict2(x_train, y_train)
    # draw_predict_font(x_train,y_train)
