...
def draw_history_loss(history):
    ...
    plt.savefig(os.path.join(imagePath, 'loss.jpg'))
    plt.show()
    plt.plot(epochs, mean_squared_error, 'b', label='训练集')
    plt.plot(epochs, val_mean_squared_error, 'g', label='验证集')
    plt.title('训练过程')
    plt.xlabel('迭代伦次')
    plt.ylabel('损失值')
    plt.legend()
...
if __name__ == '__main__':
    ...
    if iftrain:
        # 训练模型
        history = train_model(model_path, epochs_num=300, load=False)
