...
def select_dataset(path, look_back=1):
    # path = '../rl1/record/DDPG_Pendulum-v0'
    a_data_x, a_data_y = [], []
    for k in range(63):
        file_name = str(k) + '.csv'
        filepath = os.path.join(path, file_name)
        col_l = 'SGTF,dif,TOR_P,TOR_N'
        col_l = col_l.split(',')
        # df = pd.read_csv(filepath, usecols=['SGTF','TARSTF'])[['SGTF','TARSTF']]
...
def create_model(look_back, out_dim):
    ...
    model.add(layers.LSTM(128, input_shape=(look_back, in_dim), return_sequences=False))
    # model.add(layers.LSTM(128))
    model.add(layers.Dense(128))
    model.add(layers.Dense(out_dim, activation=tf.nn.tanh))
...
def train_model(model_filepath, epochs_num=100, batch_size_num=64, load=False):
    ...
    print(s_scores)
    with open(os.path.join(imagePath, "test.txt"), "w") as f:
        f.write(s_name)
...
def draw_predict_ch(scalerx, scalery, x, y):
    y_pre = model.predict(x)
    dim = y.shape[-1]
    # 画图
    plt.figure()

    plt.plot(y[:100, 0], label='真实值')
    plt.plot(y_pre[:100, 0], label='预测值')
    plt.legend()
    plt.ylabel('工具面状态')
    # plt.xlabel('测试样本点')
    plt.title('现场试验结果')
    plt.savefig(os.path.join(imagePath, 'testy——ch.jpg'))
    plt.show()

    # 画图
    plt.figure()

    plt.plot(y[:, 0], label='真实值')
    plt.plot(y_pre[:, 0], label='预测值')
    plt.legend()
    plt.ylabel('工具面状态')
    # plt.xlabel('测试样本点')
    plt.title('现场试验结果')
    plt.savefig(os.path.join(imagePath, 'testy——ch-all.jpg'))
    plt.show()
    ...
    plt.plot(ori_y[20:220, 0], label='真实值')
    plt.plot(ori_y_pre[20:220, 0], label='预测值')
    plt.legend()
    plt.title('ori_testy')
    plt.ylabel('工具面状态（°）')
    # plt.xlabel('测试样本点')
    plt.title('现场试验结果')
    plt.savefig(os.path.join(imagePath, 'ori_testy-ch.jpg'))
    plt.show()


if __name__ == '__main__':
    dataform = 'standardization'
    look_back = 20
    in_dim = 4
    out_dim = 1
    action_range = [-2, 2]
    data_path = '../truedata/true_data3'
    path = '_'.join(['timestep', str(look_back), 'outdim', str(out_dim), dataform, 'tshow-t1'])
    ...
    # 数据集
    data_x, data_y = select_dataset(data_path, look_back)
    # 划分训练集
    ...
    # 训练模型
    history = train_model(model_path, epochs_num=200, load=True)
    # filename = 'lstm1.h5'
    # model_filepath = os.path.join(model_path, filename)
    # model.load_weights('model/timestep_5_outdim_1_standardization_t0/lstm1.h5')
    ...
    draw_predict_ch(scalerx, scalery, x_c, y_c)
