...
def draw_predict_ch(scalerx, scalery, x, y):
    ...
    plt.plot(y[400:500, 0], label='真实值')
    plt.plot(y_pre[400:500, 0], label='预测值')
    plt.legend()
    plt.ylabel('工具面状态')
    # plt.xlabel('测试样本点')
    plt.title('现场试验结果')
    ...
    plt.plot(ori_y[400:500, 0], label='真实值')
    plt.plot(ori_y_pre[400:500, 0], label='预测值')
    plt.legend()
    plt.title('ori_testy')
    plt.ylabel('工具面状态（°）')
    # plt.xlabel('测试样本点')
...
if __name__ == '__main__':
    ...
    path = '_'.join(['timestep', str(look_back), 'outdim', str(out_dim),dataform,'tshow'])
