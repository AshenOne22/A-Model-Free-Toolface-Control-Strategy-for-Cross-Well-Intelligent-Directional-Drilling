...
def draw_history(history):
    ...
    plt.plot(epochs, val_mean_absolute_error, 'b', label='Validation mean_absolute_error')
    ...
    plt.show()
    plt.plot(epochs, mean_squared_error, 'bo', label='Training mean_squared_error')
...
def draw_loop(y, image_name='loop.jpg'):
    # 画图
    plt.figure()
    dim=y.shape[-1]
    for i in range(dim):
        label_name='y'+str(i)
        plt.plot(y[:, i], label=label_name)
    plt.legend()
    plt.savefig(os.path.join(loopPath, image_name))
    plt.show()
...
if __name__ == '__main__':
    ...
    out_dim = 4
    action_range = [-2, 2]
    path = '_'.join(['timestep', str(look_back), 'outdim', str(out_dim)])
    ...
    loopPath = os.path.join('loop', path)
    if not os.path.exists(loopPath):
        os.makedirs(loopPath)

    # 构建模型
    model = create_model(look_back, out_dim)
    # 加载模型
    model.load_weights(os.path.join(model_path, 'lstm1.h5'))
    ...
    state0 = x_train[index]
    state0 = state0.reshape([1, 1, 4])
    y = []
    for i in range(100):
        y0 = model.predict(state0)
        y.append(y0)
        state0 = y0.reshape([1, 1, 4])
    y = np.array(y)
    y = y.reshape(-1, 4)
    ori_y=scalery.inverse_transform(y)
    # 画图
    draw_loop(y)
    draw_loop(ori_y,'oriy_loop.jpg')
