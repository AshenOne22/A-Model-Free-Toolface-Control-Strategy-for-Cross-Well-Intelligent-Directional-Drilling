...
def plot_tf_en(data, tag='en'):
    data=data*180
    plt.figure()  # 创建一个图形实例，方便同时多画几个图
    # plt.title("Simulated directional process", fontsize=20)
    plt.xlabel('steps', fontsize=20)
    plt.ylabel('toolface orientation(°)', fontsize=20)
    plt.plot(data[:180, 0], label='toolface', linewidth=3.0)
    plt.plot(data[:180, 1], label='target toolface', linewidth=3.0)
...
def plot_tf(tf, tag='explore'):
    plt.figure()  # 创建一个图形实例，方便同时多画几个图
    plt.title("收敛阶段-工具面控制效果", fontsize=20)
    plt.xlabel('调整时间步', fontsize=20)
    plt.ylabel('角度(°)', fontsize=20)
...
def plot_rewards_single(rewards, tag='single'):
    plt.figure()  # 创建一个图形实例，方便同时多画几个图
    plt.title("测试-工具面变化", fontsize=20)
    plt.xlabel('调整时间步', fontsize=20)
    plt.ylabel('工具面角度', fontsize=20)
...
file1 = 'data/learn-ex1.csv'
dist,data = read_dist(file1)
# plot_dist(dist, '0118')
# plot_tf2(data,'0118')
plot_tf_en(data,'EN-SHOW')
