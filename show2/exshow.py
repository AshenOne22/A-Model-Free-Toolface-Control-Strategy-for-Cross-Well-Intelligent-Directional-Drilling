...
def plot_dist(dist, tag='explore'):
    ...
    plt.title("现场试验工具面控制效果", fontsize=30)
    plt.xlabel('调整时间步', fontsize=30)
    plt.ylabel('角度(°)', fontsize=30)
...
def plot_tf_en(data, tag='en'):
    plt.figure()  # 创建一个图形实例，方便同时多画几个图
    plt.title("Field experimental results", fontsize=30)
    plt.xlabel('steps', fontsize=30)
    plt.ylabel('toolface orientation(°)', fontsize=30)
...
def plot_tor(data, tag='0118tor'):
    plt.figure()  # 创建一个图形实例，方便同时多画几个图
    plt.title("现场试验算法输出参数与工程师采用参数对比", fontsize=30)
    plt.xlabel('调整时间步', fontsize=30)
    plt.ylabel('扭矩(N*m)', fontsize=30)
...
def plot_tor_en(data, tag='0118toren'):
    plt.figure()  # 创建一个图形实例，方便同时多画几个图
    plt.title("Control parameter variation curve", fontsize=30)
    plt.xlabel('steps', fontsize=30)
    plt.ylabel('torque(N*m)', fontsize=30)
